...
class Settings(BaseSettings):
    ...
    # ── Root TTL renewal ──────────────────────────────────────────
    # Anchored Merkle roots are Persistent ledger entries, so they expire unless
    # something calls `extend_root_ttl`. The write-time bump in
    # contracts/geoguard-ledger/src/storage.rs keeps a root alive for ~180 days;
    # a scheduled job renews roots before that runs out, so a batch stays
    # queryable without anyone restoring archived entries.
    #
    ...
    # Ledgers a renewal extends a root to (~180 days at 5s/ledger). Mirrors the
    # contract's ROOT_TTL_EXTEND_TO.
    ttl_renewal_extend_to_ledgers: int = 3_110_400
    # Renew once a root has less than this much life left. Must stay below the
    # contract's renewal margin (~174 days) so the call is not a silent no-op.
    ttl_renewal_window_days: int = 30
    # Most roots a single job run will renew, bounding fee spend per run.
    ttl_renewal_max_roots_per_run: int = 20
