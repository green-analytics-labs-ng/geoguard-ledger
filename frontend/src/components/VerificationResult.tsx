...
export default function VerificationResult({ result }: Props) {
  ...
  const batchVerified =
    inclusion !== undefined &&
    inclusion !== null &&
    isInclusionVerified(inclusionVerdict(inclusion.verified_locally, inclusion.verified_on_chain));
  ...
  if (match) {
    ...
    description = "The dataset hash matches a record on the Stellar blockchain.";
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div>
          ...
          <p className={`text-sm ${verified ? "text-green-700" : "text-red-700"}`}>{description}</p>
        </div>
      </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <AnomalyBadge score={(on_chain_record.anomaly_score ?? 0) / 10000} size="md" />
            </div>
            <div>
