...
mod tests {
    ...
    use crate::{storage, GeoGuardLedger, GeoGuardLedgerClient};

    fn setup() -> (Env, Address, GeoGuardLedgerClient) {
        let env = Env::default();
        let contract_id = env.register_contract(None, crate::GeoGuardLedger);
    ...
    fn test_anchor_and_verify() {
        ...
        let record = client.anchor_hash(
            &submitter,
            &hash,
            &420,                           // 4.20% anomaly score
            &Symbol::new(&env, "isoforest_v1"),
        );
        ...
        // Verify a non-existent hash returns None
    ...
    fn test_extend_ttl() {
        ...
        // Should not panic — extends TTL for an existing record
